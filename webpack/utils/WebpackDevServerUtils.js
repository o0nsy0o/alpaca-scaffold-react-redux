...
function createCompiler(webpack, config, appName, onFinished) {
  ...
  compiler.plugin('invalid', (fileChangedPath) => {
    if (isInteractive && isFirstCompile) {
      clearConsole();
    }
    if (fileChangedPath) {
      console.log(`[${chalk.cyan(path.relative(process.cwd(), fileChangedPath))}] has been changed, client recompiling...\n`);
    } else {
      // for webpack v1.
  ...
  compiler.plugin('done', stats => {
    ...
    if (noErrors) {
      ...
      console.log(chalk.red('Debug server was interrupted, please fix lint error!'));
    }
    if (isSuccessful && isFirstCompile && isInteractive) {
      console.log();
      // console.log(`Starting ${chalk.bold(appName)} dev server...`);
      console.log();
